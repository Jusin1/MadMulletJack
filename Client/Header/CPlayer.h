...
public:
	...
	// normal, zoom에 따라 update, lateupdate func
private:
	_int NormalUpdate(const _float& fTimeDelta);
	_int ZoomUpdate(const _float& fTimeDelta); // zoom

	void NormalLateUpdate(const _float& fTimeDelta);
	void ZoomLateUpdate(const _float& fTimeDelta); // zoom

	// state func
private:
	// state util func
	void StateNormalSet();						// state 변화전 변수들 초기화
	void StateZoomSet();						// state 변화전 변수들 초기화 : zoom
	...
	void CountTimeZoom(const _float& fTimeDelta);	// zoom

	void Change_State(PLAYERSTATE _eState); // 이전 state 정리 후 state 전환

	void StateBegin(PLAYERSTATE _e);	// state의 begin 함수를 실행		-> 주로 변수 셋팅
	void StateEnd(PLAYERSTATE _e);		// state의 end 함수를 실행		-> update동안 실행할 작업. 또는 state 전환 조건 체크
	void StateUpdate(PLAYERSTATE _e, const _float& fTimeDelta);	// state의 on 하수를 실행 -> 변수 정리. 바꿔줄 변화

	void StateUpdateZoom(PLAYERSTATE _e, const _float& fTimeDelta);	//zoom
...
private:
	...
	void			Set_ColliderZoom(const _float& fTimeDelta); // zoom
...
private:
	...
	_float m_fNormalSpeed;		// normal speed 값 -> 이걸 기준으로 dash 속도 잡음
	_float m_fDashCoolTime;		// dash cool time : 이중 dash 방지
	_float m_fAttackCoolTime;	// attck cool time : attack state 꼬임 방지
