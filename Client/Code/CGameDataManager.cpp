...
CGameDataManager::CGameDataManager()
{
	...
	m_vecSortedSlideWallEntries.reserve(15);
}
...
HRESULT CGameDataManager::Bind_FloorList(list<Engine::CGameObject *> *_pFloorlist)
{
	...
	Sort_List_ByZValue(m_vecSortedFloorEntries);

	return S_OK;
}

HRESULT CGameDataManager::Bind_SideWallList(list<Engine::CGameObject *> *_pSlideWalllist)
{
	if (!_pSlideWalllist)
	{
	...
	for (list<Engine::CGameObject *>::iterator itr = _pSlideWalllist->begin();
		itr != _pSlideWalllist->end();
		++itr)
	{
		if (!(*itr))
			continue;
		...
		if (!pTransform || !pBuffer)
		{
			MSG_BOX("CGameDataManager::Bind_SideWallList, Transform and Buffer was invalid");
			return E_FAIL;
		}
		...
		m_vecSortedSlideWallEntries.push_back(tEntry);
	}

	Sort_List_ByZValue(m_vecSortedSlideWallEntries);
...
void CGameDataManager::Sort_List_ByZValue(vector<PANELENTRY> &_veclist)
{
	...
	std::sort(_veclist.begin(), _veclist.end(),
...
void CGameDataManager::Clear_SliedWallList()
{
	if (m_vecSortedSlideWallEntries.size() > 0)
	{
		m_vecSortedSlideWallEntries.clear();
	}
}
