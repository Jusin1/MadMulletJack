...
void CUI::Move_UI(const _float& fTimeDelta)
{
	...
	switch (m_tMoveInfo.eUIMove) {
		//////////////////////////  좌우
	case MV_RIGHT:
		this->GetTransform()->Move_PosRight(fTimeDelta, m_tMoveInfo.fRange, m_tMoveInfo.bStop, m_tMoveInfo.fSumRange);
		break;

	case MV_LEFT:
		this->GetTransform()->Move_PosLeft(fTimeDelta, m_tMoveInfo.fRange, m_tMoveInfo.bStop, m_tMoveInfo.fSumRange);
		break;

	case MV_RL: // range 만큼 좌우로 움직임
		this->GetTransform()->Move_RL(fTimeDelta, m_tMoveInfo.fRange, m_tMoveInfo.bStop, m_tMoveInfo.fSumRange);
		break;

		//////////////////////////  상하
	case MV_UP: // y기준으로 위 아래로 움직임
		this->GetTransform()->Move_YUp(fTimeDelta, m_tMoveInfo.fRange, m_tMoveInfo.bStop, m_tMoveInfo.fSumRange);
		break;

	case MV_DOWN: 
		this->GetTransform()->Move_YDown(fTimeDelta, m_tMoveInfo.fRange, m_tMoveInfo.bStop, m_tMoveInfo.fSumRange);
		break;

	case MV_UpDown:
		this->GetTransform()->Move_YUpDown(fTimeDelta, m_tMoveInfo.fRange, m_tMoveInfo.bStop, m_tMoveInfo.fSumRange);
		break;
	...
	case MV_ROTATIONZ: // z축 기준으로 회전
		this->GetTransform()->Rotation({0.f,0.f,1.f}, fTimeDelta);
		m_fRotSum += this->GetTransform()->GetTransformInfo().fRotationSpeed * fTimeDelta;
		break;
	}
}
