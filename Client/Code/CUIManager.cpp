    ...
    if (m_timeAutoRemoveArmed) {
        ...
        if (m_timeAutoRemoveTimer >= 2.0f) {
            ...
            if (auto* talk = dynamic_cast<CTalkUI*>(CObjectManager::GetInstance()->Clone_GameObject(
                ...
                std::vector<std::wstring> dialogues = { L"贗葬橫ж樟捱蹂...", L"謠擎 勘 爾罹萄萵啪蹂", L"謠擎 摹鷗檜望!!" };
...
void CUIManager::Create_CureEff()
{
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        //fPosY = float(rand() % 301 + WINCY * 0.5f); // wincy ~ wincy + 300 餌檜縑憮 儅撩
        fPosY = 0.f;
       
        effect->Set_UISizeAndPos(128.f, 128.f, fPosX * iDis, fPosY);
        
        // move 撢た
        _float fRange = float(rand() % 400) + 200.f; // 100 縑憮 300 餌檜
...
void CUIManager::Destory_CureEff()
{
    ...
    for (auto& pChild : m_pCureEffUI->GetChildren())
    {
        pChild->Set_Dead(true);                   // 偌羹 dead 籀葬
        m_pCureEffUI->Remove_Child(pChild);      // child縑憮 薯剪
    }
...
void CUIManager::Destory_PlayerEff_ALL()
{
    ...
    //{
    //    pChild->Set_Dead(true);                   // 偌羹 dead 籀葬
    //    m_pPlayerEffUI->Remove_Child(pChild);      // child縑憮 薯剪
    //}
